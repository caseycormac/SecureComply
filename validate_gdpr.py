def validate_gdpr_record(record):
    ...
    if value != EMPTY and str(value).lower() != "na" and not isinstance(value, bool):
    ...
    if dsar_days != EMPTY and str(dsar_days).lower() != "na" and (
    not isinstance(dsar_days, int) or not (1 <= dsar_days <= 99)
):
    ...
    if breach_hours != EMPTY and str(breach_hours).lower() != "na" and (
        not isinstance(breach_hours, int) or not (1 <= breach_hours <= 168)):
    ...
    if retention_days != EMPTY and str(retention_days).lower() != "na" and (
    not isinstance(retention_days, int) or not (0 <= retention_days <= 365)):
    ...
    if (
    record.get("privacy_policy_present") is False and
    record.get("privacy_policy_clarity") not in ["missing", EMPTY, "na"]
):
    ...
    if record.get("data_retention_policy") == "none" and record.get("retention_period_days") not in [0, EMPTY, "na"]:
